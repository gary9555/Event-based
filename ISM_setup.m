function [SetupStruc] = ISM_setup(roomDim, src, rcv, fs)
...
SetupStruc.room = roomDim;        % room dimensions in m

SetupStruc.mic_pos = rcv;%[1.81  0.33  0.1;    % [x y z] positions of a microphone array in m
                      %1.88  0.4  0.1];                    
SetupStruc.src_traj = src;%[3.3  4.7  0.1];      
...
SetupStruc.T60 = 0.2;%0.2;                 % reverberation time T60, or define a T20 field instead!
% SetupStruc.T20 = 0.15;                % reverberation time T20, or define a T60 field instead!
%SetupStruc.abs_weights = [0.6*ones(6,1)];
%SetupStruc.abs_weights = [.9 .9 .9 .9 .9 .9];
...
%% Uncomment the following for a 3D plot of the above setup:
  ...
  box on; xlabel('x-axis (m)'); ylabel('y-axis (m)'); zlabel('z-axis (m)');hold off;
